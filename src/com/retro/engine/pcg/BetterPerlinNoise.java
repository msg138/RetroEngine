...
public class BetterPerlinNoise extends Noise {
    ...
    public BetterPerlinNoise(int repeat) {
        this.repeat = repeat;
    }
    ...
    public double OctavePerlin(double x, double y, double z) {
        ...
        for(int i=0;i<getOctaves();i++) {
            total += perlin(x * frequency + getSeed(), y * frequency + getSeed(), z * frequency + getSeed()) * amplitude;
    ...
    public NoiseSample getNoise3D(Vector3 v){
        return new NoiseSample((float)OctavePerlin(v.getX(), v.getY(), v.getZ()), v);
    }
